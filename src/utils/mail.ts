...
export const sendResetPasswordEmail = async (email: string, resetLink: string, username?: string): Promise<void> => {
  const mailOptions = {
    from: {
      name: 'Saudi Research Data Repository Initiative',
      address: env.EMAIL_USER
        ...
        <html>
          ...
          <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background-color: #f8f9fa; padding: 30px; border-radius: 10px;">
              <h2 style="color: #333; text-align: center;">Email Verification</h2>
              ${username ? `<p>Hello ${username},</p>` : '<p>Hello,</p>'}
              <p>Your reset password link to reset the password of your account for the Saudi Research Data Repository Initiative application is:</p>
              <div style="text-align: center;">
                <a href="${resetLink}" style="display: inline-block; background-color: #007bff; color: white; font-size: 24px; font-weight: bold; text-align: center; padding: 15px; border-radius: 5px; margin: 20px 0; text-decoration: none;">
                  Reset Password
                </a>
              </div>
              <p style="color: #666; font-size: 14px;">
